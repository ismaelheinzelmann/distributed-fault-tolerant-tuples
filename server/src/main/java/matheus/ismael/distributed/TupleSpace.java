...
public class TupleSpace implements Receiver, Runnable {
    ...
    TupleSpace() throws Exception {
        InputStream serverInputStream = TupleSpace.class.getClassLoader().getResourceAsStream("SERVER.xml");
        ...
        InputStream clientInputStream = TupleSpace.class.getClassLoader().getResourceAsStream("CLIENT.xml");
    ...
    public void receive(MessageBatch batch) {
        ...
        if (message instanceof GetStateMessage){
            ...
            synchronized (tupleSpaceLock) {
                serverChannel.send(new ObjectMessage(batch.getSender(), new StateMessage(tupleSpace)));
            }
            tupleSpaceLock.unlock();
            ...
            synchronized (tupleSpaceLock) {
                var matchingPattern = getMatchingPattern(String.join(",", ((StoreTuple) message).getTuple()));
                if (matchingPattern.isPresent()) {
                    getQueue.remove(matchingPattern.get());
                    clientChannel.send(new ObjectMessage(matchingPattern.get().getVal2(), new ReturnTuple(((StoreTuple) message).getTuple())));
                } else {
            ...
            for (Address address: clientChannel.getView().getMembers()){
                if (address.toString().equals(((GetTupleQueueMessage)message).getAddressName())){
                    getQueue.add(new Tuple<>(((GetTupleQueueMessage)message).getPattern(), address));
                }
            }
            ...
            synchronized (tupleSpaceLock){
                Lock tupleSpaceLock = lockService.getLock("tuple-spaces");
                tupleSpaceLock.lock();
                var matchingTuple = getMatchingTuple(String.join(",", ((GetTupleMessage) message).getTuple()));
                if (matchingTuple.isPresent()) {
                    tupleSpace.remove(matchingTuple.get());
                    clientChannel.send(new ObjectMessage(batch.getSender(), new ReturnTuple(matchingTuple.get())));
//                    serverChannel.send(new ObjectMessage(null, new RemoveTuple(((GetTupleMessage) message).getTuple())));
                } else {
                    // ENVIAR O ADDRESS PARA OS OUTROS INFORMANDO QUE ALGUEM ESPERA UMA TUPLA
                    getQueue.add(new Tuple<>(String.join(",", ((GetTupleMessage) message).getTuple()), batch.getSender()));
                    serverChannel.send(new ObjectMessage(null, new GetTupleQueueMessage(String.join(",", ((GetTupleMessage) message).getTuple()), batch.getSender().toString())));
                }
                tupleSpaceLock.unlock();
            }
        } else if (message instanceof WriteTupleMessage){
            synchronized (tupleSpaceLock) {
                var matchingTuple = getMatchingTuple(String.join(",", ((WriteTupleMessage) message).getTuple()));
                if (matchingTuple.isEmpty()) {
                    var matchingPatter = getMatchingPattern(String.join(",", ((WriteTupleMessage) message).getTuple()));
                    Lock tupleSpaceLock = lockService.getLock("tuple-spaces");
                    tupleSpaceLock.lock();
                    if (matchingPatter.isPresent()){
                        getQueue.remove(matchingPatter.get());
                        clientChannel.send(new ObjectMessage(matchingPatter.get().getVal2(), new ReturnTuple(((WriteTupleMessage) message).getTuple())));
                    } else {
                        tupleSpace.add(((WriteTupleMessage)message).getTuple());
                        serverChannel.send(new ObjectMessage(null, new StoreTuple(((WriteTupleMessage)message).getTuple())));
                    }
                    tupleSpaceLock.unlock();
                }
            ...
            var matchinTuple = getMatchingTuple(String.join(",", (((ReadTupleMessage) message).getTuples())));
            if (matchinTuple.isPresent()) {
                clientChannel.send(new ObjectMessage(batch.getSender(), new ReturnTuple(matchinTuple.get())));
            } else {
                clientChannel.send(new ObjectMessage(batch.getSender(), new ReturnTuple(new ArrayList<>())));
            }
        } else if (message instanceof GetListMessage){
            Lock tupleSpaceLock = lockService.getLock("tuple-spaces");
            tupleSpaceLock.lock();
            clientChannel.send(new ObjectMessage(batch.getSender(), new GetListReturnalMessage(tupleSpace)));
            tupleSpaceLock.unlock();
        }
    ...
    private void sendGetStateRequest() throws Exception {
        Message message = new ObjectMessage(serverChannel.getView().getCoord(), new GetStateMessage());
